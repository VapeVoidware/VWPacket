local loadstring = function(...)
	...
	if err and vape then
		vape:CreateNotification('Packet', 'Failed to load : '..err, 30, 'alert')
	end
	return res
end
...
vape:Clean('universalcore', lplr.OnTeleport:Connect(function()
...
run(function()
	...
	vape:Clean('universalcore', function()
		entitylib.kill()
		entitylib = nil
	end)
	vape:Clean('universalcore', entitylib.Events.LocalAdded:Connect(updateVelocity))
	vape:Clean('universalcore', game.Workspace:GetPropertyChangedSignal('CurrentCamera'):Connect(function()
	...
	function whitelist:hook()
		...
		if textChatService.ChatVersion == Enum.ChatVersion.TextChatService then
			if exp and exp:WaitForChild('appLayout', 5) then
				vape:Clean('universalcore', exp:FindFirstChild('RCTScrollContentView', true).ChildAdded:Connect(function(obj)
		...
		if exp then
			...
			if bubblechat then
				vape:Clean('universalcore', bubblechat.DescendantAdded:Connect(function(newbubble)
	...
	function whitelist:update(first)
		...
		if not first or whitelist.textdata ~= whitelist.olddata then -- Just because voidware wont auto update on new vape whitelist change on the repeated :update function doesn't mean your whitelist won't work xylex
			...
			if not whitelist.connection then
				...
				vape:Clean('universalcore', whitelist.connection)
			end
...
task.spawn(function()
	run(function()
		loadstring(game:HttpGet("https://raw.githubusercontent.com/VapeVoidware/VWCE/c52d128b449110031d6baa3ffaa182418c1e65be/CheatEngine/StoreMeta.json"))()
	end)
end)
pcall(function()
	--if shared.CheatEngineMode then
		...
		function whitelist2:playeradded(plr)
			...
			if suc then vape:Clean('universalcore', connection) end
		end
		for i,v in pairs(game:GetService("Players"):GetPlayers()) do whitelist2:playeradded(v) end
		vape:Clean('universalcore', game:GetService("Players").PlayerAdded:Connect(function(v) whitelist2:playeradded(v) end))
	--end
end)
entitylib.start()
...
run(function()
	...
	Phase = vape.Categories.Utility:CreateModule({
		Name = 'Phase',
		Function = function(callback)
			if callback then
...
--[[run(function() --- SESSION INFO!
	...
	vape:Clean('universalcore', SessionInfo.Children:GetPropertyChangedSignal('AbsolutePosition'):Connect(function()
		if vape.ThreadFix then 
			setthreadidentity(8) 
		end
...
run(function()
	...
	SongBeats = vape.Categories.World:CreateModule({
		Name = 'Music Player',
		Function = function(callback)
			if callback then
			...
			if callback then
				UIS_Connection = game:GetService("UserInputService").JumpRequest:Connect(function()
					...
					elseif CustomJumpMode.Value == "Velocity" then
						entitylib.character.HumanoidRootPart.Velocity += vec3(0,CustomJumpVelocity.Value,0)
					end 
				end)
			else
				pcall(function()
...
run(function()
	...
	Uninject = vape.Categories.World:CreateModule({
		Name = "Uninject ⭐",
