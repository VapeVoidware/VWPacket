...
for _, folder in {'vwpacket', 'vwpacket/games', 'vwpacket/profiles', 'vwpacket/assets', 'vwpacket/libraries', 'vwpacket/guis'} do
...
if not shared.VapeDeveloper then
	local _, subbed = pcall(function()
		return game:HttpGet('https://github.com/VapeVoidware/VWPacket')
	end)
	local commit = subbed:find('currentOid')
	commit = commit and subbed:sub(commit + 13, commit + 52) or nil
	commit = commit and #commit == 40 and commit or 'main'
	if commit == 'main' or (isfile('vwpacket/profiles/commit.txt') and readfile('vwpacket/profiles/commit.txt') or '') ~= commit then end
	writefile('vwpacket/profiles/commit.txt', commit)
end
...
if (not isfolder('vwpacket')) then makefolder('vwpacket') end
...
if (not isfolder('vwpacket/Libraries')) then makefolder('vwpacket/Libraries') end
local baseDirectory = "vwpacket/"
local function install_profiles(num)
    ...
    task.spawn(function()
        local res1
        if num == 1 then
            res1 = "https://api.github.com/repos/"..repoOwner.."/contents/Packet"
        end
...
commit = '6b23585308921c6266fa3c9af561df7a4bd0839a'
...
local function vapeGithubRequest(scripturl, isImportant)
    ...
    local url = 'https://raw.githubusercontent.com/VapeVoidware/VWPacket/'
    suc, res = pcall(function() return game:HttpGet(url..commit.."/"..scripturl, true) end)
    if not suc or res == "404: Not Found" then
        if isImportant and not shared.VoidDev then
...
local function pload(fileName, isImportant, required)
    print(fileName, isImportant, required)
